...
export default function Home() {
  ...
  const handleListitem = async (item) => {
    ...
    let response = await fetch(`http://localhost:3000/api/customer?location=${item}`);
    let result = await response.json();
    console.log(result);
    setDisplays(result.result)
    setShow(false);
  ...
  const handlewrite = async (e) => {
    let response = await fetch(`http://localhost:3000/api/customer?restaurant=${event.target.value}`);
    let result = await response.json();
    console.log(result);
    setDisplays(result.result)
  }
